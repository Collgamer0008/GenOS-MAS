    ...
    def create_plan(self, user_input: str, comprehension_data: Dict[str, Any], error_context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        **Examples:**
        ...
        if error_context:
            ...
            Please generate a revised plan that fixes the error and continues the task.
    ...
    def execute_commands(self, plan: Dict[str, Any], user_query: str) -> Dict[str, Any]:
        ...
        for i, cmd in enumerate(commands, 1):
            ...
            try:
                ...
                # stop execution and return error context for replanning
                return {
                    "status": "failed",
                     "total_commands": len(plan.get("steps", [])),
                    "executed_commands": i + 1,   # commands attempted so far
                     "results": execution_results,           # list of {command, status, output/error}
                    "error_context": {
                        "command": cmd,
                        "error": str(e),
...
class MultiAgentOrchestrator:
    ...
    def process_request(self, user_input: str):
        ...
        attempt=0
        while True:
            execution = self.execution_agent.execute_commands(plan, user_input)

            if execution["status"] == "success":
                print("✅ Plan executed successfully")
                add_to_kb(user_input, plan)
                break
            
            # If failure, send feedback to planner
            print("❌ Plan failed, replanning...")
            error_context = execution["error_context"]
            plan = self.planner_agent.create_plan(user_input, comprehension_data, error_context=error_context)
            attempt+=1
            # Summary
             # Summary
            ...
            print(f"Total Commands: {execution.get('total_commands', 0)}")
            print(f"Executed Commands: {execution.get('executed_commands', 0)}")

            results = execution.get('results', [])
            successful_commands = sum(1 for r in results if r['status'] == 'success')
            failed_commands = execution.get('executed_commands', 0) - successful_commands
